...
**Status:** 🚀 Produção — 1761 testes passando (100%) | 4 regimes + IMUNE + DIFAL + Cronograma + PDF educativo + Auditoria Documental LGPD + IDOR Guard + Stack front-back sincronizada + Fase 0a/0b + Fase 2 subfase 2.2 (mapa-mestre 34 categorias) + Fase 3' subfase 0 (interface FonteCliente) + WS6 etapa 4 (engine IMUNE) + WS6 etapa 5a (overlay COOPERATIVA) + WS6 etapa 6 (orquestrador societário) + Cache local de fontes normativas
...
**Data Certificação:** 08/05/2026 (WS6 etapa 6 — orquestrador `validar_combinacao()` + caller real `motor.obter_validacao_societaria()` + concatenação de falhas + Rail R7 alerta migração)
...
| **WS6 etapa 6** — orquestrador societário | (pendente PR) | `validar_combinacao()` em `core/orquestrador_societario.py` agrega matriz societária (WS6) + sub-validador MEI + sub-validador COOPERATIVA + regras CNAE (WS12) + limites versionados (WS10). **Concatena TODAS as falhas** (não pára na primeira). Caller real plugado no motor: `_validacao_societaria` no `__init__` + `obter_validacao_societaria()` + eventos `ALERTA_VALIDACAO_SOCIETARIA_*` na trilha unificada. Rail R7 (≥ 90% teto) implementado. Resultado frozen Pydantic V2 com `bloqueios`/`pendencias`/`alertas`/`engine_recomendado`/`overlay_aplicavel`/`anexo_simples_resolvido`. 26 testes (22 puros + 4 integração com motor) |

### ⏸️ Próximas etapas (ao retomar)

| Fase | Estado | Próxima ação |
| --- | --- | --- |
| **WS6 etapa 5b — cooperativa CRÉDITO + SAÚDE** | ⏸️ **RETOMAR AQUI** | LC 214/2025 Cap II Tít V (Art. 181+) regime serviços financeiros + Lei 9.718/98 Art. 14 II (Real obrigatório) + Art. 188 (reverter deduções) + Art. 192 § 8º (coop-associado fora da base sempre) + Art. 197 I (LC 227/2026 — associado tomador não credita); SAUDE: regime específico operadora plano de saúde. Schema já bloqueia subtipos CREDITO/SAUDE com mensagem 5b. Pendência registrada PMD: unificar enum `RamosCooperativos5a/5b` × Literal do schema antes de migrar. Caller real (orquestrador `validar_combinacao()`) JÁ entregue na etapa 6 |
