...
| **WS7c — INs RFB + Resolução CGSN** | ⏸️ **RETOMAR AQUI** | Capturas falharam (não-Planalto SPA — curl retorna shell HTML). Pendentes: IN RFB 2.003/2021 (ECD), IN RFB 2.004/2021 (ECF prazo), IN RFB 2.005/2021 (DCTFWeb prazo), IN RFB 1.252/2012 (EFD-Contribuições), IN RFB 1.371/2013, Lei 4.502/64 (IPI), Resolução CGSN 140/2018 (prazos Simples). Caminhos: WebFetch ou `https://www.in.gov.br/`. Após cache: validar prazos `prazo_amparo_pendente=True`, adicionar ECD/EFD-Contribuições à matriz. Plus: caller de produção pra `gerar_alertas_obrigacoes()` (motor.trilha_auditoria ou endpoint `/obrigacoes/{cnpj}`) — sem caller, abstração morta latente |
