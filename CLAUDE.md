...
**Última certificação:** 09/05/2026 (WS7c FECHADO — Chrome MCP capturou 5/6 normas Receita Federal; DCTFWeb prazo alterado p/ último dia útil mês seguinte por IN RFB 2.248/2025; EFD-Contribuições adicionada à matriz; EFD-ICMS-IPI permanece fora de escopo unificado)
...
| ~~WS7b + WS7c — Planalto + ECF/ECD/DCTFWeb + INs RFB via Chrome MCP~~ | ✅ **Fechado em 09/05/2026** | Ver bloco "Concluído neste ciclo" acima. **Claude in Chrome MCP** renderizou JS dinâmico do sped.rfb.gov.br/SPA Receita Federal — capturou 5/6 normas: IN RFB 2.003/2021 (ECD prazo último dia útil junho), IN RFB 2.004/2021 (ECF prazo último dia útil julho), IN RFB 1.252/2012 (EFD-Contribuições prazo 10º dia útil 2º mês subsequente), IN RFB 2.248/2025 (DCTFWeb prazo ALTERADO p/ último dia útil mês seguinte — antes era dia 15), Resolução CGSN 140/2018 (DASN-SIMEI/DEFIS/PGDAS-D — confirmação multi-fonte oficial). Texto extraído em `data/fontes_legais/cache_chrome/*.txt` com SHA-256 em HASHES.txt. EFD-ICMS-IPI permanece fora de escopo (estadual variável por UF). **prazo_amparo_pendente=False** em todas as 6 obrigações da matriz. Pendência residual: caller de produção pra `gerar_alertas_obrigacoes()` (motor.trilha_auditoria ou endpoint `/obrigacoes/{cnpj}`). |
