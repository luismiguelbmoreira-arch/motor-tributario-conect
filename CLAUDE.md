...
| **WS6 etapa 6** — orquestrador societário | `1ae79a2`+`e2b21c8` | `validar_combinacao()` em `core/orquestrador_societario.py` agrega matriz societária (WS6) + sub-validador MEI + sub-validador COOPERATIVA + regras CNAE (WS12) + limites versionados (WS10). **Concatena TODAS as falhas** (não pára na primeira). Caller real plugado no motor: `_validacao_societaria` no `__init__` + `obter_validacao_societaria()` + eventos `ALERTA_VALIDACAO_SOCIETARIA_*` na trilha unificada. Rail R7 (≥ 90% teto) implementado. Resultado frozen Pydantic V2 com `bloqueios`/`pendencias`/`alertas`/`engine_recomendado`/`overlay_aplicavel`/`anexo_simples_resolvido`. Hardening posterior: `fornecedora: Any` → `EmpresaFornecedora` via TYPE_CHECKING. 26 testes |
| **WS6 etapa 5b** — cooperativa CRÉDITO + SAÚDE | (pendente PR) | **CRÉDITO**: Real obrigatório (Lei 9.718/98 Art. 14 II); regime específico de serviços financeiros (LC 214 Art. 181 caput); cooperativa de crédito como entidade supervisionada SFN (Art. 183 § 1º III — **NÃO Art. 182 § 1º III**, citação inventada bloqueada por Escrivão antes do código); operações coop-associado fora da base sempre (Art. 192 § 8º, independe Art. 271); reversão de deduções proporcionais quando opt-in (Art. 188); associado tomador NÃO apropria créditos (Art. 197 I, redação LC 227/2026). **SAÚDE**: regime específico Cap III Tít V (LC 214 Arts. 234-238 — **NÃO `~12049`**, placeholder do rascunho original substituído pelo dispositivo real); base de cálculo Art. 235; alíquota referência reduzida em 60% (Art. 237); vedado crédito ao adquirente salvo PJ contratante (Art. 238 § único, redação LC 227/2026); Art. 271 (alíquota zero) **inaplicável**. PMD #1 fechado: criado `core/cooperativa_ramos.py` como fonte única (Literal + frozenset sincronizados). 17 testes novos. Cálculo automático IBS/CBS dos regimes específicos fica para etapa futura (Rail R2) |
